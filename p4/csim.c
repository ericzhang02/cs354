 ...
 */
void init_cache() {
    S = (int)pow(2, s); //block size in bytes: B = 2^b
    B = (int)pow(2, b); //number of sets: S = 2^s

    cache = malloc(S * sizeof(cache_set_t));
    //check if cache is null
    if (cache == NULL){
        printf("cashe malloc failed");
        exit(1);
    }
    ...
    for (int set = 0; set < S; set++){
        ...
        //check if null
        if (cache[set] == NULL){
            printf("cashe line malloc failed");
            exit(1);
        }
        //initialize values
...
void access_data(mem_addr_t addr) {
    //get tag and set bits
    mem_addr_t set = (addr >> b) & ((1 << s)-1);
    mem_addr_t tag = addr >> (s+b);
    //var to track data
    cache_line_t *mostUsed = &cache[set][0];
    cache_line_t *leastUsed = &cache[set][0];
    cache_line_t *hit = NULL;
    cache_line_t *empty = NULL;
    //loop through lines in set to find hit, most used line, least used line, a empty line
    for (int line = 0; line < E; line++){
        //line is valid
        if (cache[set][line].valid == '1'){
            ...
            //tracking for most used line
            if (cache[set][line].counter > mostUsed->counter) {
                mostUsed = &cache[set][line];
            }   
            //tracking for least used line 
            if (cache[set][line].counter < leastUsed->counter) {
                leastUsed = &cache[set][line];
            }
        }
        //if not valid then set is empty
        if (cache[set][line].valid == '0' && empty == NULL){
            empty = &cache[set][line];
        }
    }
    //case for if a hit is found
    if (hit != NULL){
        hit->counter = mostUsed->counter+1;
        hit_cnt++;
    }
    //case for is hit is not found and there is space in set
    else if (empty == NULL){
        leastUsed->counter = mostUsed->counter + 1;
        leastUsed->valid = '1';
        leastUsed->tag = tag;
        miss_cnt++;
        evict_cnt++;
    }
    //case for hit is not found and there is not space in set
    else if (empty != NULL){
        empty->counter = mostUsed->counter + 1;
        empty->tag = tag;
        empty->valid = '1';
        miss_cnt++;
    }
